...
def get_numbers_view(request):
    ...
    if url is None or first_page is None or end_page is None:
        return JsonResponse(
            ({"error": "Missing parameters"}), content_type="application/json"
        )
    _result = get_numbers(
        url,
    ...
    return JsonResponse(
        _result, content_type="application/json", status=200, safe=False
    )
...
def get_numbers(request):
    ...
    if MacID.objects.filter(mac_id=mac_id).exists():
        ...
        d = [{"phone": i.phone_number} for i in n]

        return JsonResponse({"message": "success", "data": d}, safe=False)
